...
struct Fifo
{
    void prepare (int numChannels, int numSamples)
    {
        static_assert (std::is_same_v<T, juce::AudioBuffer<float>>, 
                       "prepare (int numChannels, int numSamples) should only be used when the Fifo is holding juce::AudioBuffer<float>");
    ...
    void prepare (size_t numElements)
    {
        static_assert (std::is_same_v<T, std::vector<float>>, 
                       "prepare (size_t numElements) should only be used when the Fifo is holding std::vector<float>");
...
struct SingleChannelSampleFifo
{
    ...
    void update (const BlockType& buffer)
    {
        ...
        jassert (nuffer.getNumChannels() > channelToUse);
